...
import {
  ...
  extractFiles,
  saveHTMLToFile, createDirectoryIfNotPresentAndWriteJsonFile,
...
describe("FileHandler", () => {
  ...
  const invalidDirectory: string ='./notValid/test/04_utils/testDirectory/existing'
  ...
  context("getJsonFromFile", ()=> {
    ...
    it('should return json from an existing file', function () {
      const existingFilePath: string = "src/test/03_integrator/scripts-to-integrate/json-script.json"
  ...
  context("createDirectoryIfNotPresentAndWriteJsonFile", () => {
    it('should create the directory and write the JSON file', () => {
      createDirectoryIfNotPresentAndWriteJsonFile(invalidDirectory, fileName, testData);
